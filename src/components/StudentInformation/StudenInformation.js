...
class StudenInformation extends React.Component {
  ...
  handlerChangeDeDondeProviene(provinencia) {
    this.setState({provinencia})
  }
  handlerChangeInstitutoAnterior(institutoAnterior) {
    this.setState({institutoAnterior})
  }
 
  handleChangeNoCuenta (event) {
   this.setState({NoCuenta:event});
 }

 handlerChangeSeccion(seccion) {
   this.setState({seccion})
 }


 handlerChangeModalidad(modalidad) {
   this.setState({modalidad})
 }

 handlerChangerMatriculaYear(event) {
    this.setState({matricula: event});

 }

 handlerChangeRne(rne) {
   this.setState({rne})
 }
 ...
 handlerChangeLastName(lastname) {
   this.setState({lastname})
 }

 handlerChangeCurso(curso) {
   console.log(curso);
   console.log(this.state);
   this.setState({curso})
 }

 handlerCurrrentPapers(paper) {
   console.log(paper)
 }
