...
import { EmployeeUpdateModel, SalaryType } from '../../models/EmployeeModel';
...
export class EditEmployee extends Component<EditEmployee.Props> {
  ...
  statusOnChange = () => this.setState({ isActive: !this.state.isActive });
  ...
  handleFormReset = async () => {
    const answer = await confirmDialog.fire({
      title: text.discardChanges,
      text: text.cancelChangesQuestion,
      type: 'warning',
  ...
  handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await this.props.actions.updateEmployee(this.state.employeeId, payload);
  }
              ...
              >
                <div className={style['e-emp-status']}>
                  ...
                  <select
                    ...
                    value={this.state.isActive === true ? 'Active' : 'Disabled'}
                  >
