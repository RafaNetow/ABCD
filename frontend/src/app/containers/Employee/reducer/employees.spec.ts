...
describe('Employees Reducer', () => {
  ...
  describe(EmployeeActions.Type.UPDATE_EMPLOYEE_REQUEST, () => {
    it('Should set the isFetching flag to true', () => {
      const state: EmployeeState = {
        ...initialState,
        isFetching: false,
      };
      const action: any = EmployeeActions.updateEmployeeRequest();
      const mutatedState = employeeReducer(state, action);
      expect(mutatedState.isFetching).toBeTruthy();
    });
  });
  describe(EmployeeActions.Type.UPDATE_EMPLOYEE_FAILURE, () => {
    it('Should update the state error message', () => {
      const state: EmployeeState = {
        ...initialState,
        isFetching: true,
      ...
      const error = 'Ups! Something went wrong :)';
      const action: any = EmployeeActions.updateEmployeeFailure(error);
      const mutatedState = employeeReducer(state, action);
      expect(mutatedState.isFetching).toBeFalsy();
    });
  });
  describe(EmployeeActions.Type.UPDATE_EMPLOYEE_SUCCESS, () => {
    it('Should set the isFetching flag to false', () => {
      const state: EmployeeState = {
        ...initialState,
        isFetching: true,
      };
      const action: any = EmployeeActions.updateEmployeeSuccess(true);
      const mutatedState = employeeReducer(state, action);
      expect(mutatedState.isFetching).toBeFalsy();
    });
  });
